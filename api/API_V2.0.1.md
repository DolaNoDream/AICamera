...
前端在完成图片上传（至 OSS 或图床）后，将图片的公网直链 URL 及用户的成文需求发送给后端。后端结合大模型视觉分析与各平台专属语境算法，一次性返回 3条不同切入点 的优质排版文案，供用户挑选。
...
image|string[]|是|存放图片公网 URL的数组（大模型原生支持多图并发读取）|["https://xxx.oss.com/1.jpg"]|
...
```json
{
    ...
    "image": ["https://img.picui.cn/free/2024/10/25/671b5a5b5b035.jpg"],
    "requirement": {
        "type": "朋友圈",
	"location": "桂林日月双塔",
	"weather": "夜风微凉",
